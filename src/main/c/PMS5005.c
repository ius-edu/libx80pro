struct PMS5005
{
	...
	const byte ALL_POSITION_CTRL = 4;
	 const byte PWM_CTRL = 5;
	 const byte ALL_PWM_CTRL = 6;
	 const byte PARAM_SET = 7;
	
	/* SubCommands under PARAM_SET */
	 const byte DC_POSITION_PID = 7;     // positon PID Control
	 const byte DC_VELOCITY_PID = 8;     // velocity PID Control
	 const byte DC_SENSOR_USAGE = 13;
	 const byte DC_CTRL_MODE = 14;
	
	 const byte POWER_CTRL = 22;
	 const byte LCD_CTRL = 23;
	 const byte VELOCITY_CTRL = 26;
	 const byte ALL_VELOCITY_CTRL = 27;
	 const byte SERVO_CTRL = 28;
	 const byte ALL_SERVO_CTRL = 29;
	 const byte TOGGLE_DC_MOTORS = 30;
	 const byte CONSTELLATION_CTRL = 80;
	 const byte GET_MOTOR_SENSOR_DATA = 123;
	 const byte GET_CUSTOM_SENSOR_DATA = 124;
	 const byte GET_STANDARD_SENSOR_DATA = 125;
	 const byte GET_ALL_SENSOR_DATA = 127;
	...
	/* End Data ID (DID) descriptor listing */
	 ...
	 const short NON_CTRL_CMD = (short) 0xffff; // no ctrl command
	 const short NO_CTRL = (short) 0x8000;
     ...
     */
	static byte[] motorSensorRequest(short packetNumber)
	{
     ...
     */
	static byte[] standardSensorRequest(short packetNumber)
	{
		byte[] cmd = new byte[10];
     ...
     */
	static byte[] customSensorRequest(short packetNumber)
	{
		byte[] cmd = new byte[10];
     ...
     */
	static byte[] allSensorRequest(short packetNumber)
	{
		byte[] cmd = new byte[10];
     ...
     */
	static byte[] dcMotorPositionTimeCtrl(byte channel, short cmdValue, short time)
	{
     ...
     * @see getSensorPot
     */
	static byte[] dcMotorPositionNonTimeCtrl(byte channel, short cmdValue)
	{
     ...
     */
	static byte[] dcMotorPwmTimeCtrl(byte channel, short cmdValue, short time)
	{
		byte[] packet = new byte[14];
		packet[0] = STX0;	packet[1] = STX1;
		packet[2] = 1;		packet[3] = 0;
     ...
     */
	static byte[] dcMotorPwmNonTimeCtrl(byte channel, short cmdValue)
	{
		byte[] packet = new byte[12];
		packet[0] = STX0;	packet[1] = STX1;
		packet[2] = 1;		packet[3] = 0;
     ...
     */
	static byte[] servoTimeCtrl(byte channel, short cmdValue, short time)
	{
     ...
     * @see servoTimeCtrl
     */
	static byte[] servoNonTimeCtrl(byte channel, short cmdValue)
	{
