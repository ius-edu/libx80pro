...
static Buffer motorSensorRequest(short packetNumber)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[7] = PMS5005.calcCRC(buf);
...
static Buffer enableMotorSensorSending()
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[4] = PMS5005.GET_MOTOR_SENSOR_DATA_DID;
        buf->data[5] = 0; // len
        buf->data[6] = PMS5005.calcCRC(buf);
...
{
    return standardSensorAry[channel + PMS5005.ULTRASONIC_OFFSET];
}
...
static short getSensorIRRange(short channel, const int standardSensorAry[], const int customSensorAry[])
{
    ...
    {
        result = standardSensorAry[PMS5005.STANDARD_IR_RANGE_OFFSET + 1] << 8 | standardSensorAry[PMS5005.STANDARD_IR_RANGE_OFFSET];
    }
    else
    {
        result = customSensorAry[2*(channel-1) + PMS5005.CUSTOM_IR_RANGE_OFFSET + 1] << 8 | standardSensorAry[2*(channel-1) + PMS5005.CUSTOM_IR_RANGE_OFFSET];
    }
...
{
    return standardSensorAry[PMS5005.TILTING_X_OFFSET + 1] << 8 | standardSensorAry[PMS5005.TILTING_X_OFFSET];
}
...
{
    return standardSensorAry[PMS5005.TILTING_Y_OFFSET + 1] << 8 | standardSensorAry[PMS5005.TILTING_Y_OFFSET];
}
...
{
    return standardSensorAry[PMS5005.TEMPERATURE_AD_OFFSET + 1] << 8 | standardSensorAry[PMS5005.TEMPERATURE_AD_OFFSET];
}
...
{
    return standardSensorAry[PMS5005.IR_COMMAND_OFFSET + index];
}
...
{
    return standardSensorAry[2*channel + PMS5005.BATTERY_SENSOR_OFFSET + 1] << 8 | standardSensorAry[2*channel + PMS5005.BATTERY_SENSOR_OFFSET];
}
...
{
    return standardSensorAry[PMS5005.REFERENCE_VOLTAGE_OFFSET + 1] << 8 | standardSensorAry[PMS5005.REFERENCE_VOLTAGE_OFFSET];
}
...
{
    return standardSensorAry[PMS5005.POTENTIOMETER_POWER_OFFSET + 1] << 8 | standardSensorAry[PMS5005.POTENTIOMETER_POWER_OFFSET];
}
...
{
    return motorSensorAry[2*channel + PMS5005.POTENTIOMETER_SENSOR_OFFSET] + 1 << 8 | motorSensorAry[2*channel + PMS5005.POTENTIOMETER_SENSOR_OFFSET];
}
...
{
    return (motorSensorAry[2*channel + PMS5005.MOTOR_CURRENT_SENSOR_OFFSET + 1] << 8 | motorSensorAry[2*channel + PMS5005.MOTOR_CURRENT_SENSOR_OFFSET]) / 728.0;
}
...
static Buffer setMotorPolarity(unsigned char channel, unsigned char polarity)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[9] = PMS5005.calcCRC(buf);                 //Checksum
...
static Buffer enableDCMotor(unsigned char channel)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[7] = channel;          //0=L | 1=R
        buf->data[8] = PMS5005.calcCRC(buf);      //Checksum
...
static Buffer setDCMotorPositionPID(unsigned char channel, short kp, short kd, short ki_x100)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[7] = channel;                    //0=L | 1=R
        buf->data[8] = PMS5005.KP_ID;                       //Proportional gain
        buf->data[9] = kp;
        buf->data[10] = kp >> 8;
        buf->data[11] = PMS5005.KD_ID;                       //Derivative gain
        buf->data[12] = kd;
        buf->data[13] = kd >> 8;
        buf->data[14] = PMS5005.KI_ID;                       //Integral gain
        buf->data[15] = ki_x100;
        buf->data[16] = ki_x100 >> 8;
        buf->data[17] = PMS5005.calcCRC(buf);                //Checksum
...
static Buffer setDCMotorVelocityPID(unsigned char channel, short kp, short kd, short ki_x100)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[4] = PMS5005.PARAM_SET_DID;                  //DID
        buf->data[5] = 11;                         //LEN
        buf->data[6] = PMS5005.DC_VELOCITY_PID_DID;            //Subcommand
        buf->data[7] = channel;                    //0=L | 1=R
        buf->data[8] = PMS5005.KP_ID;                       //Proportional gain
        buf->data[9] = kp;
        buf->data[10] = kp >> 8;
...
static Buffer setDCMotorPositionWithTime(unsigned char channel, short pos, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[11] = PMS5005.calcCRC(buf);		       //Checksum
 ...
 * @see getSensorPot
 */
static Buffer setDCMotorPosition(unsigned char channel, short packetValue)
{
    Buffer buf = new_Buffer(12);
 ...
 */
static Buffer setDCMotorPulse(unsigned char channel, short packetValue)
{
    Buffer buf = new_Buffer(12);
...
static Buffer setAllDCMotorPositionsWithTime(short pos0, short pos1, short pos2, short pos3, short pos4, short pos5, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[20] = PMS5005.calcCRC(buf);                       //Checksum
...
static Buffer setAllDCMotorPositions(short pos0, short pos1, short pos2, short pos3, short pos4, short pos5)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[17] = pos5 >> 8;
        buf->data[18] = PMS5005.calcCRC(buf);                       //Checksum
...
static Buffer setBothDCMotorPositionsWithTime(short posLeft, short posRight, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[10] = PMS5005.NO_CTRL;               //channel 3
        buf->data[11] = PMS5005.NO_CTRL >> 8;
        buf->data[12] = PMS5005.NO_CTRL;               //channel 4
        buf->data[13] = PMS5005.NO_CTRL >> 8;
        buf->data[14] = PMS5005.NO_CTRL;               //channel 5
        buf->data[15] = PMS5005.NO_CTRL >> 8;
        buf->data[16] = PMS5005.NO_CTRL;
        buf->data[17] = PMS5005.NO_CTRL >> 8;
...
static Buffer setBothDCMotorVelocitiesWithTime(short vLeft, short vRight, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[9] = vRight >> 8;
        buf->data[10] = PMS5005.NO_CTRL;		//MOTOR 3
        buf->data[11] = PMS5005.NO_CTRL >> 8;
        buf->data[12] = PMS5005.NO_CTRL;		//MOTOR 4
        buf->data[13] = PMS5005.NO_CTRL >> 8;
        buf->data[14] = PMS5005.NO_CTRL;		//MOTOR 5
        buf->data[15] = PMS5005.NO_CTRL >> 8;
        buf->data[16] = PMS5005.NO_CTRL;		//MOTOR 6
        buf->data[17] = PMS5005.NO_CTRL >> 8;
        buf->data[18] = timePeriod;		//time
        buf->data[19] = timePeriod >> 8;
        buf->data[20] = PMS5005.calcCRC(buf);				//Checksum
...
static Buffer setServoPulseWithTime(unsigned char channel, short pulse, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[12] = PMS5005.calcCRC(buf);                       //Checksum
...
static Buffer setServoPulse(unsigned char channel, short pulse)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[8] = pulse >> 8;   //high 8 bit
        buf->data[9] = 6;                                 //flag
        buf->data[10] = PMS5005.calcCRC(buf);                       //Checksum
...
static Buffer setBothServoPulsesWithTime(short pLeft, short pRight, short timePeriod)
{
    ...
    if (buf != NULL)
    {
        ...
        buf->data[14] = PMS5005.NO_CTRL;					//channel 5
        buf->data[15] = PMS5005.NO_CTRL >> 8;
        buf->data[16] = PMS5005.NO_CTRL;					//channel 6
        buf->data[17] = PMS5005.NO_CTRL >> 8;
        buf->data[18] = timePeriod;					//time
        buf->data[19] = timePeriod >> 8;
        buf->data[20] = PMS5005.calcCRC(buf);							//Checksum
...
/*     /\* methods *\/ */
/*     self->PMS5005.calcCRC = PMS5005.calcCRC; */
